...
function validateFileName(filePath, fileName, dirPath) {
  ...
  if (dirPath.includes('/materials/ch_')) {
    ...
    if (chapterMatch && fileName.endsWith('.md')) {
      ...
      if (!itemPattern.test(fileName)) {
        errors.push(`❌ ${relativePath}: Item files should follow pattern "item_X_이름.md"`);
      }
    }
  }
  ...
  if (dirPath.includes('/images/') && !fileName.startsWith('ch_')) {
    warnings.push(`⚠️  ${relativePath}: Image files should be named with chapter prefix (e.g., ch_2_1_item_1)`);
  }
}
...
function validateDirectoryStructure(dirPath) {
  ...
  if (dirPath.includes('materials/') && dirPath !== 'materials' && !dirPath.endsWith('/materials')) {
    ...
    if (materialsIndex >= 0 && materialsIndex < pathParts.length - 1) {
      ...
      if (!subDir.match(/^ch_\d+$/)) {
        errors.push(`❌ ${relativePath}: Materials subdirectories should follow "ch_X" pattern, found "${subDir}"`);
      }
    }
  }
}
